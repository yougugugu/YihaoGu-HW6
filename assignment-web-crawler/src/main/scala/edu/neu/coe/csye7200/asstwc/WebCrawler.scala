...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[Try[URL]] = {
      for(nss <- ns \\ "a"; href = nss\@"href") yield createURL(Some(u), href)
   }
    // TO BE IMPLEMENTED
    ...
    // 9 points.
    for(urls <- getURLContent(u); links <- MonadOps.asFuture(getLinks(urls))) yield links // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    Future.sequence(for(us2s <- us2) yield MonadOps.sequence(wget(us2s)))
  }
